...
def getModel(name):
    ...
    model = TGCN(adj, num_quantiles = len(quantiles_list), horizon=TIMESTEP_OUT).to(device)
    return model
...
def V1_calModel(name, mode, XS, YS, quantiles_list, scaler,cal_q):
...
#   KEYWORD = 'pred_' + DATANAME + '_' + MODELNAME + '_' + 'adaptive' + '_' + datetime.now().strftime(
#        "%y%m%d%H%M")
#     print(KEYWORD, 'cal started', time.ctime())
#     calXS, calYS = getXSYS_single(data, 'CAL')
#     print('CAl XS.shape YS,shape', calXS.shape, calXS.shape)
#     err = V2_calModel(MODELNAME, 'cal', calXS, calYS, scaler, cal_list)
#     print(KEYWORD, 'cal ended', time.ctime())
# # for our method test
#     print(KEYWORD, 'testing started', time.ctime())
#     testXS, testYS = getXSYS_single(data, 'TEST')
