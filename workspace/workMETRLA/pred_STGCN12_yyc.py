...
def getModel(name):
    ks, kt, bs, T, n, p = 3, 3, [[CHANNEL, 16, 64], [64, 16, 64]], TIMESTEP_IN, N_NODE, DROPOUT
...
def mc_dropout_predictModel(model, data_iter):
    YS_pred = []
    model.eval()
...
def mcdropout_testModel(name, mode, XS, YS, mc_sample_num=10):
    print('Model Testing Started ...', time.ctime())
    print('TIMESTEP_IN, TIMESTEP_OUT', TIMESTEP_IN, TIMESTEP_OUT)
    XS_torch, YS_torch = torch.Tensor(XS).to(device), torch.Tensor(YS).to(device)
    test_data = torch.utils.data.TensorDataset(XS_torch, YS_torch)
    ...
    for i in range(mc_sample_num):
        YS_pred_ = np.squeeze(mc_dropout_predictModel(model, test_iter))
        YS_pred_ = scaler.inverse_transform(YS_pred_)
...
KEYWORD = 'Ypred_' + DATANAME + '_' + MODELNAME + '_dropout_' + datetime.now().strftime("%y%m%d%H%M")
# PATH = '../save/' + KEYWORD
PATH = '../save/Ypred_METR-LA_STGCN12_dropout_2210181713'
...
def main():
    ...
    testXS, testYS = getXSYS_single(data, 'TEST')
    print('TEST XS.shape, YS.shape', testXS.shape, testYS.shape)
    mcdropout_testModel(MODELNAME, 'test', testXS, testYS)
