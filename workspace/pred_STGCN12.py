...
def getModel(name, dropout):
    ks, kt, bs, T, n, p, dropout = 3, 3, [[CHANNEL, 16, 64], [64, 16, 64]], TIMESTEP_IN, N_NODE, 0, dropout
    ...
    model = STGCN(ks, kt, bs, T, n, Lk, p, dropout).to(device)
    return model
...
def trainModel(name, mode, XS, YS, scaler, drop):
    ...
    model = getModel(name, dropout=drop)
