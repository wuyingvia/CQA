...
def cal_model_1(name, mode, XS, YS,  scaler, cal_q):
    ...
    model.load_state_dict(torch.load('/home/wuying/CODE/mymodel/2022/DL-Traff-Graph/v3_save/pred_PEMS08_MTGNN_quantile_conformal_2210262249/MTGNN.pt'))

    YS_pred = predictModel(model, cal_iter)

    YS_pred_0 = YS_pred[..., 0][:,-1,:]
    YS_pred_1 = YS_pred[..., 1][:,-1,:]
    ...
    index = np.argmin(np.abs(m_coverage - 0.92))
    return  [err[0][index], err[1][index]]

def cal_model_2(name, mode, XS, YS,  scaler, lambda_list):
    YS = YS[:,-1,:]
    XS = XS
    XS_torch, YS_torch = torch.Tensor(XS).to(device), torch.Tensor(YS).to(device)
    cal_data = torch.utils.data.TensorDataset(XS_torch, YS_torch)
    ...
    err_dis = np.concatenate([error_low,error_high])
    error_quantile = 101
    corr_err_list = []
    for j in range(1, error_quantile):
        corr_err = np.quantile(err_dis, 1 / (error_quantile - 1) * j, axis=0)
        corr_err = torch.Tensor(corr_err).to(device)
    ...
    for i in range(len(lambda_list)):
        ...
        corr_err = [torch.stack(corr_err_list)[index[t], t] for t in range(node)]
        corr_err = torch.stack(corr_err).cpu().numpy()
        corr_u_err.append(corr_err)

    err = np.stack(corr_u_err)
    independent_coverage_l = []
    for i in range(len(lambda_list)):
        y_u_pred = YS_0_val + err[i]
        y_l_pred = YS_1_val - err[i]
        ...
        independent_coverage_l.append(m_coverage)
    m_coverage = np.stack(independent_coverage_l)
    index = np.argmin(np.abs(m_coverage - 0.912))

    return  [err[index], err[index]]
...
def ftestunModel_1(name, mode, XS, YS,scaler):
    ...
    model = getModel(name)
    model.load_state_dict(torch.load(PATH + '/' + name + '.pt'))
    model.load_state_dict(torch.load('/home/wuying/CODE/mymodel/2022/DL-Traff-Graph/results/表格的结果/pred_METR-LA_MTGNN_quantile_conformal_2210262326/MTGNN.pt'))
...
def ftestunModel_2(name, mode, XS, YS, quantiles_list, err, scaler, cal_list):
    ...
    YS_pred =np.squeeze(predictModel(model, test_iter))        

    YS, YS_pred_0, YS_pred_1 = np.squeeze(YS), YS_pred[...,0],YS_pred[...,1]
