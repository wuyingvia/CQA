...
def testunmodel(name, mode, XS, YS, sample):
    print('Model Testing Started ...', time.ctime())
    print('TIMESTEP_IN, TIMESTEP_OUT', TIMESTEP_IN, TIMESTEP_OUT)
    XS_torch, YS_torch = torch.Tensor(XS).to(device), torch.Tensor(YS).to(device)
    test_data = torch.utils.data.TensorDataset(XS_torch, YS_torch)
    ...
    model = getModel(name)
    model.load_state_dict(torch.load(PATH + '/' + name + '.pt'))
    #model.load_state_dict(torch.load('/home/wuying/CODE/mymodel/2022/DL-Traff-Graph/v1_save/bayesianpred_pred_PEMS-BAY_STGCN3_2206271000/STGCN3.pt'))
    if LOSS == 'MSE':
        criterion = nn.MSELoss()
    if LOSS == 'MAE':
        criterion = nn.L1Loss()
...
def main():
    ...
    testXS, testYS = getXSYS_single(data, 'TEST')
    print('TEST XS.shape, YS.shape', testXS.shape, testYS.shape)
    testunmodel(MODELNAME, 'test', testXS, testYS, sample)
