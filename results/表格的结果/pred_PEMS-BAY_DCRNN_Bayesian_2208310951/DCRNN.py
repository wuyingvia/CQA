...
class GCN(nn.Module):
    def __init__(self, DEVICE, prior_mu, prior_sigma,K: int, input_dim: int, hidden_dim: int, bias=True, activation=nn.ReLU):
        super().__init__()
        self.DEVICE = DEVICE
    ...
    def init_params(self, n_supports: int, b_init=0):
        ...
        self.weight_mu = nn.parameter.Parameter(torch.empty(n_supports * self.input_dim, self.hidden_dim), requires_grad=True)
        self.weight_log_sigma = nn.parameter.Parameter(torch.empty(n_supports * self.input_dim, self.hidden_dim), requires_grad=True)
        self.register_buffer('weight_eps', None)

        self.bias_mu = nn.Parameter(torch.empty(self.hidden_dim), requires_grad=True)
        self.bias_log_sigma = nn.Parameter(torch.empty(self.hidden_dim), requires_grad=True)
        self.register_buffer('bias_eps', None)

        if self.weight_eps is None :
            self.W = self.weight_mu + torch.exp(self.weight_log_sigma) * torch.randn_like(self.weight_log_sigma)
        else :
            self.W = self.weight_mu + torch.exp(self.weight_log_sigma) * self.weight_eps
        if self.bias :
            if self.bias_eps is None:
                self.b = self.bias_mu + torch.exp(self.bias_log_sigma) * torch.randn_like(self.bias_log_sigma)
            else:
                self.b = self.bias_mu + torch.exp(self.bias_log_sigma) * self.bias_eps
    ...
    def reset_parameters(self):
        ...
        self.W = torch.Tensor(np.random.normal(loc=self.prior_mu,scale=self.prior_sigma,size=self.W.data.shape)).to(self.DEVICE)
        self.b = torch.Tensor(np.random.normal(loc=self.prior_mu,scale=self.prior_sigma,size=self.b.data.shape)).to(self.DEVICE)
...
class DCGRU_Cell(nn.Module):
    def __init__(self, DEVICE, prior_mu, prior_sigma, num_nodes: int, input_dim: int, hidden_dim: int, K: int, bias=True, activation=None):
        super(DCGRU_Cell, self).__init__()
        self.num_nodes = num_nodes
        self.input_dim = input_dim
        self.hidden_dim = hidden_dim

        self.conv_gate = GCN(DEVICE,prior_mu, prior_sigma, K=K,
                             input_dim=input_dim + hidden_dim,
                             hidden_dim=hidden_dim * 2,  # for update_gate, reset_gate
                             bias=bias,
                             activation=activation)
        self.conv_cand = GCN(DEVICE, prior_mu, prior_sigma, K=K,
                             input_dim=input_dim + hidden_dim,
...
class DCGRU_Encoder(nn.Module):
    def __init__(self, DEVICE, prior_mu, prior_sigma, num_nodes: int, input_dim: int, hidden_dim, K: int, num_layers: int,
        ...
        for i in range(self.num_layers):
            ...
            self.cell_list.append(DCGRU_Cell(DEVICE, prior_mu = prior_mu,  prior_sigma = prior_sigma , num_nodes=num_nodes,
...
class DCGRU_Decoder(nn.Module):  # projected output as input at the next timestep
    def __init__(self, DEVICE, prior_mu, prior_sigma, num_nodes: int, out_horizon: int, out_dim: int, hidden_dim, K: int, num_layers: int,
        ...
        for i in range(self.num_layers):
            ...
            self.cell_list.append(DCGRU_Cell(DEVICE, prior_mu = prior_mu, prior_sigma = prior_sigma,
                                             num_nodes=num_nodes,
                                             input_dim=cur_input_dim,
                                             hidden_dim=self.hidden_dim[i],
                                             K=K,
        ...
        self.out_projector = torchbnn.BayesLinear(prior_mu=self.prior_mu,prior_sigma=self.prior_sigma,
...
class DCRNN(nn.Module):
    def __init__(self, device, prior_mu, prior_sigma, num_nodes: int, input_dim: int, out_horizon: int, P: list, K=3, hidden_dim=64, num_layers=2, bias=True,
                 activation=None):
        super(DCRNN, self).__init__()
        self.K = K
        self.P = self.compute_cheby_poly(P).to(device)  
        self.encoder = DCGRU_Encoder(DEVICE = device, prior_mu = prior_mu, prior_sigma = prior_sigma, num_nodes=num_nodes, input_dim=input_dim, hidden_dim=hidden_dim, K=self.P.shape[0],
                                     num_layers=num_layers, bias=bias, activation=activation, return_all_layers=True)
        self.decoder = DCGRU_Decoder(DEVICE = device,prior_mu = prior_mu, prior_sigma= prior_sigma, num_nodes=num_nodes, out_dim=input_dim, hidden_dim=hidden_dim, K=self.P.shape[0],
...
def main():
    ...
    model = DCRNN(device, prior_mu=prior_mu,prior_sigma=prior_sigma, num_nodes=N_NODE, input_dim=CHANNEL, out_horizon=TIMESTEP_OUT, P=adj_mx).to(device)
