...
def getModel(name, drop):
    ...
    model = gwnet(device, num_nodes=N_NODE, drop = drop, in_dim=CHANNEL, supports=supports, dropout = 0.0).to(device)
    return model
...
def testModel(name, mode, XS, YS, drop, scaler):
    if LOSS == "MaskMAE":
        criterion = Utils.masked_mae
    if LOSS == 'MSE':
        criterion = nn.MSELoss()
...
parser.add_argument('--dropout', type=float, default='0.1') # quantile/quantile_conformal/adaptive
...
def main():
    ...
    testModel(MODELNAME, 'test', testXS, testYS, drop, scaler)
