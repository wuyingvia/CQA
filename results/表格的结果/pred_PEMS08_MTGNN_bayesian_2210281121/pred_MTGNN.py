...
def testunmodel(name, mode, XS, YS, device, sample, scaler):
    if LOSS == "MaskMAE":
        criterion = Utils.masked_mae
    if LOSS == 'MSE':
        criterion = nn.MSELoss()
...
def main():
    ...
    testunmodel(name=MODELNAME, mode = 'test', XS = testXS,YS = testYS,device = device,
                sample= sample, scaler = scaler)
